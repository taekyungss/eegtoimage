...
from EEG_encoder import Conformer
...
def train(preprocess, model, temperature, optimizer, scheduler, START_EPOCH, num_epochs, experiment_num):
    ...
    for epoch in range(START_EPOCH, num_epochs):
        ...
        for batch_idx, (EEGs, images, labels) in enumerate(tq):
            ...
            EEGs = torch.transpose(EEGs, 1,2)
            EEGs = torch.unsqueeze(EEGs, dim=1)
            EEGs = EEGs.to(config.device).float()
            labels = labels.to(config.device).long()

            optimizer.zero_grad()

            EEG_embed = model(EEGs, images)
...
# ## define the model
eeg_embedding = Conformer().to(device)
